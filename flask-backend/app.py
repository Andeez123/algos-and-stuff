...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///app.db' # change this for prod db
...
# Models
# to use flask-sql-alchemy ORM features
# each model is a python representation of db tables
# we use this to query with python instead of writing raw sql
# can use automap instead of models, for changing db schemas
class User(db.Model):
    id = db.Column(db.Integer, primary_key=True)
...
def login():
    ...
    user = User.query.filter_by(email=data['email']).first() # finds the first user with the given email
